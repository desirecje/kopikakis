...
export const Route = createFileRoute("/auth/login")({
  head: () => ({ meta: [{ title: "Log in — Kopi Kaki" }] }),
  component: LoginPage,
});
...
function LoginPage() {
  ...
  const resendCode = async () => {
    ...
    setBusy(true);
    const { error } = await supabase.auth.signInWithOtp({
      email, options: { shouldCreateUser: false, emailRedirectTo: undefined },
    });
    setBusy(false);
    ...
    <main className="min-h-screen bg-[#EDE8DC] flex flex-col items-center justify-center px-4">
      <div className="w-full max-w-sm">
        ...
        <div className="bg-[#E0D9C8] rounded-2xl border border-[rgba(92,51,23,0.15)] p-6">
          {stage === "email" ? (
            <>
              <h2 className="text-xl font-bold text-[#3A2410] mb-1">Welcome back</h2>
              <p className="text-sm text-[#7A6A55] mb-5">Enter your NUS email to receive a login code.</p>
              ...
              <p className="text-center text-xs text-[#7A6A55] mt-4">
                ...
                <button onClick={() => navigate({ to: "/auth/signup" })} className="text-[#5C3317] font-medium hover:underline">
                  Sign up
                </button>
              </p>
            </>
            ...
            <>
              <h2 className="text-xl font-bold text-[#3A2410] mb-1">Enter your code</h2>
